    ...
    def get_optimal_params(platform_type: str, mpv_version: tuple) -> List[str]:
        ...
        if platform_type == 'raspberry_pi':
            ...
            params.extend([
                ...
                '--hwdec=v4l2m2m-copy',  # V4L2 аппаратное ускорение для RPi
                '--hwdec-codecs=all',  # Использовать hwdec для всех кодеков
            ])
            
            print(f"[Detector] ✅ Используются только проверенные параметры")
            print(f"[Detector] 🚀 Аппаратное ускорение: v4l2m2m-copy (GPU декодинг)")
