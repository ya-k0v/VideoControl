    ...
    def get_optimal_params(platform_type: str, mpv_version: tuple) -> List[str]:
        ...
        if platform_type == 'raspberry_pi':
            ...
            params.extend([
                ...
                '--hwdec=mmal',  # MMAL - нативный hwdec для старых RPi
                '--hwdec-codecs=h264,hevc,mpeg2video,mpeg4',  # Основные кодеки
            ])
            
            print(f"[Detector] ✅ Используются только проверенные параметры")
            print(f"[Detector] 🚀 Аппаратное ускорение: MMAL (нативный GPU RPi)")
