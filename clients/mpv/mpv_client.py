...
class DeviceDetector:
    ...
    def get_optimal_params(platform_type: str, mpv_version: tuple) -> List[str]:
        ...
        if platform_type == 'raspberry_pi':
            ...
            # Базовые параметры уже есть в params выше
            params.extend([
                '--cache=yes',
                '--cache-secs=10',
                '--network-timeout=60',
                '--image-display-duration=inf',  # Для статических изображений
                '--pause=no',  # Автоматически играть
                '--vo=x11',  # X11 video output для стабильности
                '--hwdec=auto',  # Попробовать аппаратное ускорение
            ])
            
            print(f"[Detector] ✅ Используются только проверенные параметры + hwdec=auto")
            print(f"[Detector] 🚀 Аппаратное ускорение: auto (попробует V4L2/MMAL)")
